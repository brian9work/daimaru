    ...
    <div className='bg-white mb-2 rounded-3xl shadow-lg border border-gray-100 overflow-hidden transition-all duration-300 hover:shadow-xl'>
      ...
      >
        <div className="flex items-center gap-3">
          <span className={`w-1.5 h-6 rounded-full transition-colors duration-300 ${isOpen ? 'bg-red-500' : 'bg-gray-300'}`}></span>
          <img
            src={category.source}
            alt={category.name}
            width={40}
        ...
        <div className={`p-2 rounded-full bg-gray-50  text-gray-500 transition-transform duration-500 ${isOpen ? 'rotate-180 bg-red-50 text-red-500' : ''}`}>
          <ChevronDown size={22} />
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className='flex-1'>
        <h3 className='font-bold text-gray-900 text-lg mb-1 group-hover:text-red-600 transition-colors'>{saucer.name}</h3>
      ...
      <div className='flex flex-col items-end gap-2 shrink-0'>
        <span className='font-bold text-lg text-red-500'>${saucer.price.toFixed(2)}</span>
